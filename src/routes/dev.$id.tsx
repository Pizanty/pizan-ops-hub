...
function DevDetail() {
  ...
  return (
    <Sheet open onOpenChange={(o) => !o && nav({ to: "/dev" })}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-2xl">
        <SheetHeader><SheetTitle>{item?.title ?? "Item"}</SheetTitle></SheetHeader>
        {!item ? <div className="py-6 text-sm text-muted-foreground">Loading…</div> : (
            ...
            <div>
              ...
              {updates.length === 0 ? <div className="text-xs text-muted-foreground">No changes yet.</div> : (
                <ul className="space-y-1 text-xs">
                  {updates.map((u) => (
                    <li key={u.id} className="rounded border p-2 font-mono text-[11px]">
